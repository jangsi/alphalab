...
function priceFormat(tickItem) {
  ...
  if ( String(Number(tickItem)).length < 7  && ( Number(tickItem) > 15 ) ){
  
    return String(Number(tickItem).toFixed(0))+'K'
    
  } else if ( String(Number(tickItem)).length > 6  && ( Number(tickItem) > 15 ) ){
    return String(Number(tickItem/1000000).toFixed(0))+'M' 
  } else {
        ...
        <Card >
            <CardBody className="card-body-test">
              <FormGroup className="w-25 select2-container mb-3 d-inline-block me-2">
                <Label className="control-label">Anchor Hisotrical Timeseries</Label>
                <Select
